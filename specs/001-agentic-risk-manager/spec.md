...
- Q: Why are Beta and risk ratios showing blank or requiring drawer clicks, and how should they be surfaced? → A: Render Beta (vs. BTC), Sharpe ratio (Rf=0), and Treynor ratio (Rf=0) directly inline in the Holdings Table and Blotter Table for each position, backed by fast cached single-risk factor calculations, alongside the drill-down drawer.
